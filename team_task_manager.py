...
def character_exceed(*args):
    ''' 
    Checks if arguments exceed the length of 182 characters.

    (The 182 character limit is for the sake of presentation.
    To stay within the 200 character length border lines.
    This maximum length was determined by how when task information is 
    displayed in the console it takes up 18 characters.)
...
def reg_user():
    ...
    Conditions required for user registration:
        ...
        - The username and password can't exceed 182 characters or include ';'.

    If any of the above criteria are not met the user is notified of this and
    returned to the main menu.
    
    If the above criteria are met, the new user is added into user.txt as
...
def add_task():
    ...
    Conditions required for task assignment:
        - User inputs can't exceed 182 characters or include ';'.
...
def change_person(t_choice, all_t_index):
    '''
    Allows the admin to enter the name of the user they want the task
    reassigned to. This change will be reflected in the console and tasks.txt
    from this point onwards. After the task has been reassigned the admin is
    notified of this and returned to view all tasks on the system.
...
def vm_change_person(t_choice, index_dict, all_t_index):
    '''
    Allows the user to enter the name of the user they want the task
    reassigned to. This change will be reflected in the console and tasks.txt
    from this point onwards. After the task has been reassigned the current
    user is notified of this and returned to view all tasks assigned to them
    on the system.
...
def edit_name():
    ''' 
    Allows non-admin users to change their username stored in users.txt and
    activate any corresponding changes in tasks.txt. 
    ...
    Conditions required for username change:
        ...
        - The new username can't exceed 182 characters or include ';'.
    
    If any of the above criteria are not met the user is notified this and
    returned to the change login details menu.

    If the above criteria are met, the new username replaces the old username
    stored in user.txt. as "new_username;password" on the same line as before.
...
def edit_pass():
    ''' 
    Allows users to change their password stored in users.txt. 
    ...
    Conditions required for password change:
        ...
        - The new password and confirmed password must match.
        - The new password can't exceed 182 characters or include ';'.
    
    If any of the above criteria are not met the user is notified this and
    returned to the change login details menu.

    If the above criteria are met, the new password replaces the old username
    stored in user.txt. as "username;new_password" on the same line as before.
